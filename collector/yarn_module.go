 ...
 * ====================================================================== */
 ...
 func create_yarn_metric (ctx context.Context, config Collector_connection_data, query string, metric_struct prometheus.Desc, ch chan<- prometheus.Metric, pclient *pool.PClient) bool {
   // Make the query
 ...
 func (ScrapeYARN) Scrape (ctx context.Context, config *Collector_connection_data, ch chan<- prometheus.Metric) error {
   ...
   log.Info_msg("In the YARN Module has been executed %d queries. %d success and %d with errors", success_queries + error_queries, success_queries, error_queries)
   return nil
 }
